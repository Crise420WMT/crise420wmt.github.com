...
function tween(obj,json,options){
    ...
    var start={};
    var dis={};
    var iCount=Math.ceil(options.time/20);
    for(var name in json){
        start[name]=parseFloat(getStyle(obj,name));
        //console.log(getStyle(obj,name));
        if(isNaN(start[name])){
            switch (name){
                case 'left':
    ...
    //options.time/iCount*n;
    obj.timer=setInterval(function(){

        //console.log(n);
        n++;
        for(var name in json){
            var cur=options.type(options.time*n/iCount,start[name],dis[name],options.time);
            if(name=='opacity'){
